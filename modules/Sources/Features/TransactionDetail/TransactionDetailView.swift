...
public struct TransactionDetailView: View {
    ...
    public var body: some View {
        ScrollView([.vertical]) {
            NighthawkHeading(title: L10n.Nighthawk.TransactionDetails.title, compact: true)
                .padding(.bottom, 24)
...
private extension StoreOf<TransactionDetail> {
    func transactionLineItems(with tokenName: String) -> [TransactionLineItem] {
        ...
        // Retrieval / OMR method this transaction was discovered or built with.
        if self.syncMethod != .unknown {
            result.append(
                TransactionLineItem(
                    name: "Sync method",
                    value: self.syncMethod.displayName
                )
            )
        }
