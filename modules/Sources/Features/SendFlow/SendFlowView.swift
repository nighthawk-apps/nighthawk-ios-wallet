...
public struct SendFlowView: View {
    ...
    public var body: some View {
        NavigationStack(
            ...
            VStack {
                NighthawkHeading(title: L10n.Nighthawk.TransferTab.Send.chooseHowMuch, compact: true)

                Text(
                    L10n.Nighthawk.TransferTab.Send.spendableBalance(
                        store.spendableBalance.decimalString(
                            formatter: NumberFormatter.darkfiNumberFormatter
                        ),
                        store.tokenName
                    )
                )
                .paragraphMedium()
                .padding(.bottom, store.availableTokens.isEmpty ? 40 : 16)

                // Always show token picker when the wallet reports any token balances
                // (including a single custom token). Native DRK is always an option.
                if !store.availableTokens.isEmpty {
                    tokenPicker
                        .padding(.bottom, 24)
                }
    ...
    var availableActions: some View {
        Group {
            if store.amountToSend > .zero {
                ...
                .disabled(!store.canSendEnteredAmount)
                .padding(.bottom, 28)
