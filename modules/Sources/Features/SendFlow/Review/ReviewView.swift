...
public struct ReviewView: View {
    ...
    public var body: some View {
        ScrollView([.vertical]) {
            NighthawkHeading(title: L10n.Nighthawk.TransferTab.Review.title, compact: true)
                .padding(.bottom, 44)
    ...
    func transactionLineItems(with tokenName: String) -> [TransactionLineItem] {
        ...
        result.append(
            contentsOf: [
                ...
                // PerfOMR/OMD is the only send path (Rust default); no scheme toggle.
                TransactionLineItem(
                    name: "Sync method",
                    value: "PerfOMR (default)"
                ),
                TransactionLineItem(
