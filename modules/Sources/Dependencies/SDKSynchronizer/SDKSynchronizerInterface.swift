...
public struct SDKSynchronizerClient {
    ...
    /// `tokenId` is nil for native DRK; otherwise the selected token id.
    public var proposeTransfer: (Int, Recipient, DrkAmount, Memo?, String?) async throws -> Proposal

    /// Estimate fee for a transfer (`tokenId` nil = native DRK).
    public var estimateFee: (DarkfiAddress, DrkAmount, String?) async throws -> DrkAmount

    // MARK: - Send (via Rust core `drk` plugin)

    /// Send a transaction. Accepts spending key, amount, recipient, memo, optional tokenId.
    /// Rust always embeds PerfOMR/OMD clue + RegisterOmrClue after broadcast.
    public var sendTransaction: (Any, DrkAmount, Recipient, Memo?, String?) async throws -> DarkfiTransactionOverview
    ...
    /// Build + broadcast a DAO transfer proposal (ZK proofs on-device). Returns proposal bulla b58.
    public var daoProposeTransfer: (_ daoName: String, _ durationBlockwindows: UInt64, _ amount: String, _ tokenId: String?, _ recipientAddress: String) async throws -> String

    /// Vote yes/no on a DAO proposal (full governance token weight). Returns tx hash.
    public var daoVote: (_ proposalBullaB58: String, _ voteYes: Bool) async throws -> String
    ...
    /// Delete on-disk wallet DB + sled cache only (keeps seed / passphrase).
    /// Use after native upgrades that cannot open an older local database.
    public var nukeLocalDatabases: () async throws -> Void
