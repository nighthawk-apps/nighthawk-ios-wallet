...
extension SDKSynchronizerClient: TestDependencyKey {
    ...
    public static let testValue: SDKSynchronizerClient = Self(
        ...
        daoProposeTransfer: unimplemented("\(Self.self).daoProposeTransfer"),
        daoVote: unimplemented("\(Self.self).daoVote"),
        wipe: unimplemented("\(Self.self).wipe"),
        nukeLocalDatabases: unimplemented("\(Self.self).nukeLocalDatabases"),
    ...
    public static let inert = SDKSynchronizerClient(
        ...
        generateNewAddress: { "" },
        getAllTransactions: { [] },
        proposeTransfer: { _, _, _, _, _ in Proposal(estimatedFee: 10_000) },
        estimateFee: { _, _, _ in 10_000 },
        sendTransaction: { _, _, _, _, _ in DarkfiTransactionOverview.mocks[0] },
        getTransactionMemo: { _ in nil },
        getTransactionRecipient: { _ in nil },
        listDaos: { [] },
        listProposals: { _ in [] },
        getProposal: { _ in nil },
        daoProposeTransfer: { _, _, _, _, _ in "stub_bulla" },
        daoVote: { _, _ in "stub_tx_hash" },
        wipe: { },
        nukeLocalDatabases: { },
        rewind: {
    ...
    public static func mock(
        ...
        Self(
            ...
            proposeTransfer: { _, _, _, _, _ in
                Proposal(estimatedFee: 10_000)
            },
            estimateFee: { _, _, _ in 10_000 },
            sendTransaction: { _, amount, recipient, memo, _ in
            ...
            listProposals: { _ in [] },
            getProposal: { _ in nil },
            daoProposeTransfer: { _, _, _, _, _ in "mock_bulla" },
            daoVote: { _, _ in "mock_tx_hash" },
            wipe: { },
            nukeLocalDatabases: { },
            rewind: {
                Just(()).setFailureType(to: Error.self).eraseToAnyPublisher()
