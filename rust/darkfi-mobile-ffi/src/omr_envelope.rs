...
/// Build an OMR envelope (supports large UnifOMR RLWE clues).
pub fn wrap_envelope(omr_memo: &[u8], fhe_clue: &[u8], tx_bytes: &[u8]) -> Vec<u8> {
...
/// Strip envelope if present; otherwise return input unchanged.
...
mod tests {
    ...
    fn test_roundtrip_large_clue() {
        ...
        let env = wrap_envelope(&memo, &clue, &tx);
        ...
        assert_eq!(strip_envelope(&env), tx.as_slice());
    }
    ...
    fn rejects_invalid_om_tag() {
        ...
        assert_eq!(strip_envelope(&env), env.as_slice());
    }
}
